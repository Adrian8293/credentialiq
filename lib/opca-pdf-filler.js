...
export async function fillOpcaPdf({ templateBuffer, profile, initials }) {
  ...
  // Build a safe field map — getFieldMaybe throws on rich text fields,
  // so we pre-index all fields that are safely readable.
  const safeFields = {}
  for (const field of form.getFields()) {
    try {
      const name = field.getName()
      // Touch the field to confirm it's readable (rich text throws here)
      const fieldType = field.constructor.name
      if (fieldType === 'PDFTextField') {
        field.getText() // will throw if rich text
      }
      safeFields[name] = field
    } catch {
      // Rich text field — skip entirely
    }
  }
  ...
  for (const { field_id, value } of fieldValues) {
    try {
      const field = safeFields[field_id]
      if (!field) continue
  ...
  // Do NOT call form.flatten() — it crashes on rich text fields.
  // The PDF remains editable so the provider can complete remaining fields.
