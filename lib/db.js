...
export async function uploadDocumentFile(documentId, providerId, file) {
  ...
  const path = `${providerId}/${documentId}/${Date.now()}_${file.name.replace(/[^a-zA-Z0-9._-]/g, '_')}`
  ...
  // Persist file_url + file_name back to the document record
  const { error: updateError } = await supabase
    .from('documents')
    .update({ file_url: data.publicUrl, file_name: file.name })
    .eq('id', documentId)
