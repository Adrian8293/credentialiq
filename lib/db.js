...
export async function deleteProvider(id) {
  ...
  // BUG-012: Cascade soft-delete related records so they stop appearing in
  // active lists, the enrollment pipeline, watchdog tasks, and expiry alerts.
  // Uses Promise.allSettled so one cascade failure doesn't block the others.
  // Only stamps records that aren't already soft-deleted (avoids overwriting
  // independently-deleted records with a newer timestamp).
  ...
  if (failures.length) {
    ...
    // Don't throw — the provider is deleted; partial cascade is better than none.
    // A full reconciliation can be done by re-running the cascade via admin tools.
  }

  await addAudit('Provider', 'Deleted', `Provider ${id} (enrollments, documents, tasks cascaded)`, id)
}
...
export async function deleteDocumentFile(documentId, fileUrl) {
  ...
  // BUG-005: Previous regex /\/documents\/(.+)$/ included the ?token=... query string
  // in the extracted path, causing storage remove() to fail silently.
  // Signed URL format: /storage/v1/object/sign/documents/<path>?token=...
  // Public URL format: /storage/v1/object/public/documents/<path>
  // The [^?]+ stops at the query string delimiter in both cases.
  ...
  if (pathMatch?.[1]) {
    ...
    if (removeError) {
      // Log but don't throw — allow DB cleanup to proceed even if storage fails
...
export async function deleteDocument(id) {
  // BUG-004: Previous implementation only soft-deleted the DB row, leaving the
  // storage file orphaned indefinitely. This combined function fetches the URL
  // first, then soft-deletes, then cleans storage non-blocking.

  // Step 1: Fetch file_url before any modification (need it for storage cleanup)
  // Failure here should NOT block the delete — proceed regardless.
  let fileUrl = null
  let fileName = null
  try {
  ...
  // Step 2: Soft-delete the DB row (user-facing operation — throws on failure)
  const { error } = await supabase
    .from('documents')
    .update({ deleted_at: new Date().toISOString() })
    .eq('id', id)
  ...
  // Step 3: Clean storage non-blocking — failure is logged, never surfaces to user
  if (fileUrl) {
  ...
  await addAudit('Document', 'Deleted', fileName || id, id)
}
...
export async function clearAuditLog() {
  // HIPAA §164.312(b): audit logs are append-only. This function records the
  // archive request in the audit trail itself, then returns. Actual record
  // deletion must be performed by a privileged admin via supabaseAdmin in a
  // separate controlled process — never from an authenticated browser client.
  //
  // The DELETE attempt has been removed entirely. Previously the code attempted
  // a delete and silently swallowed the RLS error — creating a false sense that
  // the log was cleared. The UI now shows the correct state on next refresh.
