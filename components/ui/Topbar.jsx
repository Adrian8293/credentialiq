...
export function Topbar({ page, setPage, openEnrollModal, openPayerModal, openDocModal, openTaskModal, exportJSON, saving, onOpenSearch, alertCount, user, signOut }) {
  ...
  const titles = {
    ...
    'provider-lookup': 'NPI Lookup',
    'license-verification': 'License Verification',
    'psychology-today': 'Psychology Today',
    enrollments: 'Payer Hub',
    pipeline: 'Payer Hub',
    'payer-requirements': 'Payer Hub',
    payers: 'Payer Hub',
    'payer-hub': 'Payer Hub',
  ...
  const sections = {
    dashboard: 'Overview', alerts: 'Overview',
    providers: 'Providers', 'add-provider': 'Providers', 'provider-lookup': 'Providers', 'license-verification': 'Providers', 'psychology-today': 'Providers',
    'payer-hub': 'Enrollments', enrollments: 'Enrollments', pipeline: 'Enrollments', payers: 'Enrollments', 'payer-requirements': 'Enrollments',
    documents: 'Compliance', 'missing-docs': 'Compliance', workflows: 'Compliance',
    eligibility: 'Revenue Cycle', claims: 'Revenue Cycle', denials: 'Revenue Cycle', revenue: 'Revenue Cycle',
    reports: 'Analytics', audit: 'Analytics',
    settings: 'System',
  }

  function topCTA() {
    if (page === 'payer-hub') return
    ...
    <div className="topbar">
      ...
      <div className="topbar-left">
        <span className="topbar-crumb">
          {sections[page] || 'Home'} &rsaquo; {titles[page] || page}
        </span>
      ...
      <div className="topbar-actions">
        ...
        {/* Page CTA */}
        {ctaLabel && (
          <button className="btn btn-primary btn-sm" onClick={topCTA}>
            {ctaLabel}
          </button>
        )}
        ...
        <div className="topbar-icon-btn" title="Alerts & Notifications" onClick={() => setPage('alerts')}>
          ...
          {alertCount > 0 && (
            <span className="topbar-notif-badge">{alertCount > 9 ? '9+' : alertCount}</span>
          )}
        </div>
        ...
        <div style={{position:'relative'}} ref={userMenuRef}>
          <div className="topbar-user-btn" onClick={() => setUserMenuOpen(o => !o)}>
            ...
            <svg className="topbar-caret" width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
